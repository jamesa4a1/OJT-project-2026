        ...
        <div className="flex items-center gap-4">
          <motion.button
            ...
            className={`p-2 rounded-lg transition-colors ${
              isDark ? 'bg-slate-700/50 text-slate-400 hover:bg-slate-600 hover:text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-800'
            }`}
            title="Go back"
          >
          ...
          <div>
            <h1 className={`text-2xl font-bold ${isDark ? 'text-white' : 'text-slate-800'}`}>
              <i className="fas fa-clipboard-check mr-3 text-blue-500"></i>
              Issue Clearance
            </h1>
      ...
      >
        ...
        <div className="flex items-center gap-3 mt-4 flex-wrap">
          ...
          <motion.button
            ...
            className="px-4 py-2 rounded-lg font-semibold text-sm text-white bg-gradient-to-r from-blue-500 to-indigo-600 hover:shadow-lg transition-all"
          >
          ...
          <table className="w-full table-fixed">
            <thead>
              <tr className={isDark ? 'bg-slate-700/50' : 'bg-slate-50'}>
                ...
                <th className={`text-left py-3 px-2 text-sm font-bold uppercase tracking-wide w-[16%] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  Applicant Name
                </th>
                ...
                <th className={`text-center py-3 px-2 text-sm font-bold uppercase tracking-wide w-[12%] ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                  Actions
                </th>
              </tr>
            </thead>
                ...
                clearances.map((clearance, index) => (
                  ...
                  >
                    <td className="py-3 px-2">
                      <span className={`font-mono text-sm font-semibold truncate block ${isDark ? 'text-blue-400' : 'text-blue-600'}`}>
                        {clearance.or_number}
                      </span>
                    </td>
                    <td className="py-3 px-2">
                      <span className={`text-sm font-semibold truncate block ${isDark ? 'text-white' : 'text-slate-800'}`}>
                        {[clearance.first_name, clearance.middle_name ? `${clearance.middle_name.charAt(0)}.` : '', clearance.last_name].filter(Boolean).join(' ')}
                      </span>
                    </td>
      ...
      <AnimatePresence>
        {showDetailModal && selectedClearance && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-start justify-center py-8 p-4"
          ...
          >
            <motion.div
              ...
              className={`w-full max-w-xl ${isDark ? 'bg-slate-800' : 'bg-white'}`}
              style={{ borderRadius: '20px', overflow: 'hidden' }}
              onClick={(e) => e.stopPropagation()}
            >
              <div className={`px-4 py-3 border-b ${isDark ? 'border-slate-700 bg-slate-800' : 'border-slate-200 bg-white'}`}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className={`text-sm mt-1 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
                      Format: <span className="font-semibold">{selectedClearance.format_type}</span> - {FORMAT_LABELS[selectedClearance.format_type]}
                    </p>
                  </div>
                  <button
                    onClick={() => setShowDetailModal(false)}
              ...
              <div className="p-3 flex justify-center bg-slate-100 dark:bg-slate-900">
                {/* Certificate Preview Container */}
                <div 
                  className="bg-white text-black rounded-lg shadow-lg transform scale-75"
                  style={{ 
                ...
                >
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '20px' }}>
                    ...
                    <div style={{ flex: 1, textAlign: 'center', fontFamily: '"Century Gothic", Arial, sans-serif', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                      ...
                      <p style={{ 
                        ...
                        Email: <a href="mailto:ocptagbilaran@doj.gov.ph" style={{ 
                          color: '#0000FF', 
                  ...
                  <div style={{ 
                    ...
                    <p style={{ 
                      ...
                      {selectedClearance.issued_by_name}
                    </p>
                  ...
                  <div style={{ 
                    ...
                    <p style={{ margin: '0 0 4pt 0' }}>
                      <span style={{ display: 'inline-block', width: '60pt', fontWeight: 'normal' }}>O.R No</span>
                      <span style={{ marginRight: '8px' }}>:</span>
                      <strong>{selectedClearance.or_number}</strong>
                    </p>
                    <p style={{ margin: '0 0 16pt 0' }}>
                      <span style={{ display: 'inline-block', width: '60pt', fontWeight: 'normal' }}>Date</span>
                      <span style={{ marginRight: '8px' }}>:</span>
                      <span>{new Date(selectedClearance.date_issued).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
                    </p>
                  </div>
                  ...
                  <div style={{ 
                    ...
                    <p style={{ margin: '0' }}>
                      <strong>Note:</strong> Valid until 6 months from the date issued.
                    </p>
                  </div>
                </div>
              </div>
