            ...
            >
                            ...
                            <div className="flex items-center gap-4">
                                <div className={`w-14 h-14 rounded-xl flex items-center justify-center
                                               ${user?.role === 'Admin' ? 'bg-gradient-to-br from-red-500 to-red-600' : 
                                                 user?.role === 'Clerk' ? 'bg-gradient-to-br from-amber-500 to-amber-600' : 
                                                 'bg-gradient-to-br from-blue-500 to-blue-600'} shadow-lg`}>
                                    <i className={`fas ${user?.role === 'Admin' ? 'fa-shield-halved' : 
                                                  user?.role === 'Clerk' ? 'fa-pen-to-square' : 'fa-eye'} 
                                                  text-white text-xl`}></i>
                                </div>
                                <div>
                                    <p className="text-sm text-slate-500 m-0">Welcome back,</p>
                                    <h2 className="text-xl font-bold text-slate-800 m-0">{user?.name || user?.email}</h2>
                                    <p className={`text-sm font-medium m-0
                                                  ${user?.role === 'Admin' ? 'text-red-600' : 
                                                    user?.role === 'Clerk' ? 'text-amber-600' : 'text-blue-600'}`}>
                                        {user?.role} Dashboard
                                    </p>
                                </div>
                            </div>
                                ...
                                <div className="hidden md:flex items-center gap-2 px-4 py-2 rounded-xl bg-slate-50 border border-slate-200">
                                    ...
                                    <span className="text-sm font-medium text-slate-700">
                                        {user?.role === 'Admin' ? 'Full Access' : 
                                         user?.role === 'Clerk' ? 'Create, View & Edit' : 'View Only'}
                                    </span>
                                </div>
                                <motion.button
                ...
                >
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-xl 
                        ...
                        <span>Tip: Use <kbd className="px-2 py-0.5 bg-white rounded border text-xs">keyboard navigation</kbd> for faster access</span>
                    </div>
                </motion.div>
            </motion.div>
        </div>
