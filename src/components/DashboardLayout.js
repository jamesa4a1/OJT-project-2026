...
const DashboardLayout = ({ children }) => {
    ...
    return (
        <div className="h-screen w-screen bg-slate-100 flex flex-col overflow-hidden">
            {/* Top Header */}
            <header className={`h-16 flex-shrink-0 px-8 flex items-center shadow-lg z-50 transition-colors duration-300 ${isDark ? 'bg-slate-900 border-b border-slate-700' : 'bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800'}`}>
            ...
            <div className="flex-1 flex overflow-hidden relative">
                ...
                {/* Main Content */}
                <main className={`flex-1 overflow-x-hidden overflow-y-auto px-6 py-8 transition-colors duration-300 ${isDark ? 'bg-slate-900' : 'bg-slate-100'}`}>
