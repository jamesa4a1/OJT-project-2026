...
const Navbar = () => {
    ...
    const { theme, toggleTheme } = useContext(ThemeContext) || { theme: 'dojlight', toggleTheme: () => {} };
                    ...
                    <div className="hidden md:flex items-center gap-2">
                        {navItems.map((item, index) => (
                            ...
                            >
                                ...
                                >
                                    <i className={`fas ${item.icon} ${isActive(item.path) ? 'text-white' : 'text-blue-400'}`}></i>
                                    {item.label}
                                    {isActive(item.path) && (
                                        <motion.div
                                            layoutId="activeTab"
                        ...
                        {isAuthenticated ? (
                            <div className="flex items-center gap-2 ml-2">
                                ...
                                <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-slate-700/50 border border-slate-600/50">
                                    <div className={`w-7 h-7 rounded-lg bg-gradient-to-br ${roleColors[user?.role]?.bg || 'from-blue-500 to-blue-600'} 
                                                    flex items-center justify-center`}>
                                        <i className={`fas ${roleColors[user?.role]?.icon || 'fa-user'} text-white text-xs`}></i>
                                    </div>
                                    <span className="text-slate-300 text-sm font-medium hidden lg:inline">
                                        {user?.name || 'User'}
                                    </span>
                                    <span className={`text-xs px-1.5 py-0.5 rounded ${roleColors[user?.role]?.badge || 'bg-blue-500'} text-white`}>
                                        {user?.role}
                                    </span>
                                </div>
                                {/* Logout Button */}
                                <motion.button
                                    ...
                                    whileTap={{ scale: 0.95 }}
                                    onClick={() => { logout(); window.location.href = '/homepage'; }}
                            ...
                            {isAuthenticated ? (
                                <div className="space-y-3">
                                    ...
                                    <div className="flex items-center gap-3 px-4 py-3 rounded-xl bg-slate-700/50">
                                        <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${roleColors[user?.role]?.bg || 'from-blue-500 to-blue-600'} 
                                                        flex items-center justify-center`}>
                                            <i className={`fas ${roleColors[user?.role]?.icon || 'fa-user'} text-white`}></i>
                                        </div>
                                        <div>
                                    ...
                                    {/* Logout Button */}
                                    <button
                                        onClick={() => { logout(); setIsOpen(false); window.location.href = '/homepage'; }}
                                        className="flex items-center gap-3 px-4 py-3 rounded-xl font-medium
                                ...
                                <div className="space-y-2">
                                    <Link to="/login" onClick={() => setIsOpen(false)}>
                                        <button className="flex items-center gap-3 px-4 py-3 rounded-xl font-medium
